...
result = pd.DataFrame((W @ data.T), index=["Y", "Y_Import", "r_c", "r_p"],
                        columns=df.index)
print(result)
...
rect1 = ax.bar(index, result["EU"], bar_width, alpha=0.5,color='y',yerr=0.1 * result["EU"].std(), error_kw=error_config, label="EU")

rect2 = ax.bar(index + bar_width, result["India"], bar_width, alpha=0.5, color='b',
               yerr=0.1 * result["India"].std(), error_kw=error_config, label="India")
...
ax.hlines(0, -0.5, 3.5, colors='k', linestyles="dashed")
ax.set_xticks(index)
